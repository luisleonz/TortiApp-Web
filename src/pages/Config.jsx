...
import { Card, SectionLabel, Button, Sheet, Avatar, IconButton, PlusIcon, XIcon, TrashIcon, CoinsIcon, MinusIcon, ChevRIcon } from '../components/ui'
...
export default function Config() {
  ...
  const [prestamoForm, setPrestamoForm] = useState(null) // { tipo: 'prestamo'|'credito', empId }
  ...
  return (
    <>
          ...
          <Card className="!p-0 overflow-hidden">
            ...
            <div className="divide-y divide-line">
              {empleados.filter(e => e.activo).map(e => {
                const prestamo = state.prestamos.find(p => p.empleadoId === e.id)?.saldoPendiente || 0
                const credito = state.creditoTienda.find(c => c.empleadoId === e.id)?.saldoPendiente || 0
                if (prestamo === 0 && credito === 0) return null
                return (
            ...
            <div className="p-3.5 border-t border-line">
              <button type="button" onClick={() => setPrestamoForm({ tipo: 'prestamo' })}
                className="w-full flex items-center gap-2 text-sm font-bold text-amber-600">
                <PlusIcon className="w-4 h-4" /> Registrar préstamo o crédito
              </button>
            </div>
          </Card>
        </div>
      ...
      {/* Sheet de detalle de préstamo/crédito de empleado */}
      {selectedEmpId && (
        <PrestamoDetail
          empId={selectedEmpId}
          onClose={() => setSelectedEmpId(null)}
          onRegister={(tipo) => { setPrestamoForm({ tipo, empId: selectedEmpId }); setSelectedEmpId(null) }}
        />
      )}
